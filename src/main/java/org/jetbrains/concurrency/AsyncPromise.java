...
public class AsyncPromise<T> extends Promise<T> implements Getter<T> {
    ...
    private static final class CompoundConsumer<T> implements Consumer<T> {
        ...
        public CompoundConsumer( Consumer<T> c1,  Consumer<T> c2) {
            synchronized (this) {
                consumers.add(c1);
                consumers.add(c2);
            }
    ...
    public <SUB_RESULT> Promise<SUB_RESULT> then( final Function<T, SUB_RESULT> fulfilled) {
        switch (state) {
            case PENDING:
                break;
            case FULFILLED:
    ...
    @Override
    void notify( final AsyncPromise<T> child) {
    ...
    public <SUB_RESULT> Promise<SUB_RESULT> then( final AsyncFunction<T, SUB_RESULT> fulfilled) {
        switch (state) {
            case PENDING:
                break;
            case FULFILLED:
    ...
    public Promise<T> processed( final AsyncPromise<T> fulfilled) {
        switch (state) {
            case PENDING:
                break;
            case FULFILLED:
    ...
    private void addHandlers( Consumer<T> done,  Consumer<Throwable> rejected) {
        this.done = setHandler(this.done, done);
        this.rejected = setHandler(this.rejected, rejected);
    }
    ...
    static boolean isObsolete( Consumer<?> consumer) {
    ...
    @Override
    public Promise<T> processed( final Consumer<T> processed) {
        done(processed);
