...
public abstract class AbstractIdeaSpecificSettings<T, C, SdkType> {
    public void readIdeaSpecific( Element root, T model,  SdkType projectSdkType,  Map<String, String> levels) {
    ...
    public void updateEntries(Element root, T model,  SdkType projectSdkType) {
        setupJdk(root, model, projectSdkType);
        setupCompilerOutputs(root, model);
        readContentEntry(root, model);
    }
    ...
    protected abstract void setupJdk(Element root, T model,  SdkType projectSdkType);
