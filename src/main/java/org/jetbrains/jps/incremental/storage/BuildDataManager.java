...
public class BuildDataManager implements StorageOwner {
  ...
  public <S extends StorageOwner> S getStorage( BuildTarget<?> target,  StorageProvider<S> provider) throws IOException {
  ...
  private static void wipeStorage(File root,  AbstractStateStorage<?, ?> storage) {
    if (storage != null) {
  ...
  private static void closeStorage( AbstractStateStorage<?, ?> storage) throws IOException {
    if (storage != null) {
      synchronized (storage) {
        storage.close();
      }
