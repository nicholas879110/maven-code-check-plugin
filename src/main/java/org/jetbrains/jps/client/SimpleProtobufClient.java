...
public class SimpleProtobufClient<T extends ProtobufResponseHandler> {
  ...
  public final RequestFuture<T> sendMessage(final UUID messageId, MessageLite message,  final T responseHandler,  final RequestFuture.CancelAction<T> cancelAction) {
  ...
  private void notifyTerminated(UUID messageId, RequestFuture<T> requestFuture,  T responseHandler) {
    try {
