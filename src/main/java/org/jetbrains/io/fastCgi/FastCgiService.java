...
public abstract class FastCgiService extends SingleConnectionNetService {
  ...
  public FastCgiService( Project project) {
    super(project);
  }
  ...
  private static void sendBadGateway( Channel channel) {
    try {
      if (channel.isActive()) {
  ...
  @Override
  protected void configureBootstrap( Bootstrap bootstrap,  final Consumer<String> errorOutputConsumer) {
  ...
  public void send( final FastCgiRequest fastCgiRequest,  ByteBuf content) {
  ...
  private void handleError( FastCgiRequest fastCgiRequest,  ByteBuf content) {
    try {
  ...
  void responseReceived(int id,  ByteBuf buffer) {
