...
import com.gome.maven.internal.focus.FocusTracesAction;
...
import com.gome.maven.openapi.components.impl.ServiceManagerImpl;
...
public class FocusManagerImpl extends IdeFocusManager implements Disposable {
    ...
    private boolean checkForRejectOrByPass( FocusCommand cmd, final boolean forced,  ActionCallback result) {
        if (cmd.isExpired()) {
    ...
    private void setCommand( final FocusCommand command) {
    ...
    private void resetCommand( final FocusCommand cmd, boolean reject) {
    ...
    private void resetUnforcedCommand( final FocusCommand cmd) {
        UIUtil.invokeAndWaitIfNeeded(new Runnable() {
            @Override
            public void run() {
                myFocusRequests.remove(cmd);
    ...
    @Override
    public void typeAheadUntil( ActionCallback callback) {
    ...
    @Override
    public void revalidateFocus( final ExpirableRunnable runnable) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
    ...
    private static Component getComponent( Map<IdeFrame, Component> map, IdeFrame frame) {
