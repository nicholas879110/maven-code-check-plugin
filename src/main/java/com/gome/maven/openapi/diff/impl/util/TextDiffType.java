...
import com.gome.maven.openapi.diff.ex.DiffStatusBar;
...
public class TextDiffType implements DiffStatusBar.LegendTypeDescriptor {
    ...
    public static TextDiffType create( final TextDiffTypeEnum type) {
    ...
    public static TextDiffType deriveApplied( TextDiffType source) {
        return new TextDiffType(source.myType, source.myDisplayName, source.myAttributesKey, true, false);
    }

    public static TextDiffType deriveInstanceForInlineWrapperFragment( TextDiffType source) {
    ...
    private Color getBgColorForFragmentContainingInlines( EditorEx editor) {
        TextAttributes originalAttrs = getTextAttributes(editor.getColorsScheme());
        if (originalAttrs == null) {
            return null;
        }
