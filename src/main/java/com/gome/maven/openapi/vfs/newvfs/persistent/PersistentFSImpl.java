...
import com.gome.maven.openapi.util.io.*;
import com.gome.maven.openapi.vfs.*;
import com.gome.maven.openapi.vfs.ex.temp.TempFileSystem;
import com.gome.maven.openapi.vfs.newvfs.*;
import com.gome.maven.openapi.vfs.newvfs.events.*;
import com.gome.maven.openapi.vfs.newvfs.impl.*;
import com.gome.maven.util.*;
import com.gome.maven.util.containers.ConcurrentIntObjectMap;
import com.gome.maven.util.containers.ContainerUtil;
import com.gome.maven.util.containers.EmptyIntHashSet;
import com.gome.maven.util.io.ReplicatorInputStream;
...
public class PersistentFSImpl extends PersistentFS implements ApplicationComponent {
    ...
    public PersistentFSImpl( MessageBus bus) {
        myEventBus = bus;
    ...
    private static FSRecords.NameId[] persistAllChildren( final VirtualFile file, final int id,  FSRecords.NameId[] current) {
    ...
    public FSRecords.NameId[] listAll( VirtualFile parent) {
    ...
    @Override
    public void renameFile(final Object requestor,  VirtualFile file,  String newName) throws IOException {
    ...
    private InputStream createReplicator( final VirtualFile file,
    ...
    private static List<VFileEvent> validateEvents( List<VFileEvent> events) {
    ...
    public VirtualFileSystemEntry findRoot( String basePath,  NewVirtualFileSystem fs) {
        if (basePath.isEmpty()) {
    ...
    private VirtualFileSystemEntry applyEvent( VFileEvent event) {
        try {
            if (event instanceof VFileCreateEvent) {
    ...
    private static VirtualFileSystemEntry executeCreateChild( VirtualFile parent,  String name) {
        final NewVirtualFileSystem delegate = getDelegate(parent);
        final VirtualFile fake = new FakeVirtualFile(parent, name);
        final FileAttributes attributes = delegate.getAttributes(fake);
        if (attributes != null) {
    ...
    private static int createAndFillRecord( NewVirtualFileSystem delegateSystem,
                                            VirtualFile delegateFile,
                                           int parentId,
                                            FileAttributes attributes) {
    ...
    private static void executeRename( VirtualFile file,  final String newName) {
        final int id = getFileId(file);
    ...
    private static void executeSetTarget( VirtualFile file, String target) {
    ...
    private static void executeTouch( VirtualFile file, boolean reloadContentFromDelegate, long newModificationStamp) {
    ...
    private static class JarRoot extends AbstractRoot {
        ...
        private JarRoot( NewVirtualFileSystem fs, int id, VfsData.Segment segment, VfsData.DirectoryData data, VirtualFile parentLocalFile) {
    ...
    private static class FsRoot extends AbstractRoot {
        ...
        private FsRoot( NewVirtualFileSystem fs, int id, VfsData.Segment segment, VfsData.DirectoryData data,  String basePath) {
            super(id, segment, data, fs);
