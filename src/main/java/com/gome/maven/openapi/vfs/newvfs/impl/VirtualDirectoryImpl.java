...
import com.gome.maven.openapi.vfs.impl.win32.Win32LocalFileSystem;
...
public class VirtualDirectoryImpl extends VirtualFileSystemEntry {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.vfs.newvfs.impl.VirtualDirectoryImpl");
    ...
    private VirtualFileSystemEntry findChild( String name,
                                             boolean doRefresh,
    ...
    private void addToAdoptedChildren(final boolean ignoreCase,  final String name) {
     ...
     // null if there can't be a child with this name, NULL_VIRTUAL_FILE
    private VirtualFileSystemEntry doFindChildInArray( String name, boolean ignoreCase) {
        synchronized (myData) {
    ...
    private VirtualFileSystemEntry doFindChild( String name,
                                               boolean ensureCanonicalName,
                                                NewVirtualFileSystem delegate,
                                               boolean ignoreCase) {
        if (name.isEmpty()) {
    ...
    public VirtualFileSystemEntry createChild(String name, int id,  NewVirtualFileSystem delegate) {
        synchronized (myData) {
    ...
    private VirtualFileSystemEntry createChild(int nameId, int id,  NewVirtualFileSystem delegate) {
    ...
    private void assertConsistency(boolean ignoreCase,  Object details) {
    ...
    public VirtualFileSystemEntry findChild( final String name) {
    ...
    private static int findIndex(final int[] array,  CharSequence name, boolean ignoreCase) {
        int low = 0;
    ...
    private static int compareNames( CharSequence name1,  CharSequence name2, boolean ignoreCase) {
        int d = name1.length() - name2.length();
        if (d != 0) return d;
        for (int i = 0; i < name1.length(); i++) {
            // com.gome.maven.openapi.util.text.StringUtil.compare(String,String,boolean) inconsistent
