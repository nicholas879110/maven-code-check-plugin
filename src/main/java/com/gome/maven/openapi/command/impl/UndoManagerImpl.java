...
import com.gome.maven.openapi.command.*;
import com.gome.maven.openapi.command.undo.*;
...
public class UndoManagerImpl extends UndoManager implements ProjectComponent, ApplicationComponent, Disposable {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.command.impl.UndoManagerImpl");
    ...
    public void undoableActionPerformed(UndoableAction action) {
        ...
        if (myCommandLevel == 0) {
            LOG.assertTrue(action instanceof NonUndoableAction,
                    "Undoable actions allowed inside commands only (see com.gome.maven.openapi.command.CommandProcessor.executeCommand())");
    ...
    protected boolean isUndoOrRedoAvailable( FileEditor editor, boolean undo) {
        ApplicationManager.getApplication().assertIsDispatchThread();
