...
abstract class OrderEnumeratorBase extends OrderEnumerator implements OrderEnumeratorSettings {
    ...
    public OrderEnumeratorBase( Module module,  Project project,  OrderRootsCache cache) {
        myCache = cache;
    ...
    @Override
    public OrderRootsEnumerator roots( NotNullFunction<OrderEntry, OrderRootType> rootTypeProvider) {
    ...
    @Override
    public void forEachModule( final Processor<Module> processor) {
        forEach(new Processor<OrderEntry>() {
            @Override
            public boolean process(OrderEntry orderEntry) {
    ...
    @Override
    public <R> R process( final RootPolicy<R> policy, final R initialValue) {
