...
import com.gome.maven.openapi.vcs.impl.VcsEP;

import java.util.*;

public class AllVcses implements AllVcsesI, Disposable {
    private final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.vcs.impl.projectlevelman.AllVcses");
    ...
    public void registerManually( final AbstractVcs vcs) {
        synchronized (myLock) {
    ...
    public void unregisterManually( final AbstractVcs vcs) {
        synchronized (myLock) {
            if (! myVcses.containsKey(vcs.getName())) return;
            unregisterVcs(vcs);
