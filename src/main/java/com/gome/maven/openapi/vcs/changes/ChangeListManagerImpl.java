...
import com.gome.maven.ide.highlighter.WorkspaceFileType;
import com.gome.maven.lifecycle.PeriodicalTasksCloser;
import com.gome.maven.openapi.application.ApplicationManager;
import com.gome.maven.openapi.application.ModalityState;
import com.gome.maven.openapi.application.RuntimeInterruptedException;
...
import com.gome.maven.openapi.roots.impl.DirectoryIndexExcludePolicy;
...
import com.gome.maven.openapi.vcs.changes.conflicts.ChangelistConflictTracker;
import com.gome.maven.openapi.vcs.changes.ui.CommitHelper;
import com.gome.maven.openapi.vcs.checkin.CheckinEnvironment;
import com.gome.maven.openapi.vcs.checkin.CheckinHandler;
import com.gome.maven.openapi.vcs.impl.*;
import com.gome.maven.openapi.vcs.readOnlyHandler.ReadonlyStatusHandlerImpl;
...
import com.gome.maven.vcsUtil.Rethrow;
...
public class ChangeListManagerImpl extends ChangeListManagerEx implements ProjectComponent, ChangeListOwner, JDOMExternalizable,
        RoamingTypeDisabled {
    public static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.vcs.changes.ChangeListManagerImpl");
     ...
     */
    private boolean showRemoveEmptyChangeListsProposal( final VcsConfiguration config,  Collection<LocalChangeList> lists) {
        if (lists.isEmpty()) {
            return false;
        }
...
//    @CalledInAwt
    ...
    @Override
    public void freezeImmediately( String reason) {
    ...
    @Override
    public LocalChangeList addChangeList( final String name,  final String comment,  final Object data) {
    ...
    @Override
    public void setDefaultChangeList( final LocalChangeList list) {
        ApplicationManager.getApplication().runReadAction(new Runnable() {
            @Override
            public void run() {
                synchronized (myDataLock) {
    ...
    private Set<VirtualFile> getUnversionedDescendantsRecursively( List<VirtualFile> items,
                                                                   final Condition<FileStatus> condition) {
    ...
    private Set<VirtualFile> getUnversionedParents( Collection<VirtualFile> items,  Condition<FileStatus> condition) {
    ...
    @Override
    public boolean editName( final String fromName,  final String toName) {
        return ApplicationManager.getApplication().runReadAction(new Computable<Boolean>() {
            @Override
            public Boolean compute() {
                synchronized (myDataLock) {
    ...
    @Override
    public String editComment( final String fromName, final String newComment) {
