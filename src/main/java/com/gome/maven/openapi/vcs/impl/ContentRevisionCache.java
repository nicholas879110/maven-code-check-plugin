...
import com.gome.maven.openapi.util.Throwable2Computable;
...
import com.gome.maven.openapi.vcs.changes.FilePathsHelper;
import com.gome.maven.openapi.vcs.changes.VcsDirtyScope;
...
public class ContentRevisionCache {
    ...
    private void put(FilePath path, VcsRevisionNumber number,  VcsKey vcsKey,  UniqueType type,  final byte[] bytes) {
    ...
    private boolean putCurrent(FilePath path, VcsRevisionNumber number,  VcsKey vcsKey, final long counter) {
        synchronized (myLock) {
    ...
    public static byte[] getOrLoadAsBytes(final Project project, FilePath path, VcsRevisionNumber number,  VcsKey vcsKey,
                                           UniqueType type, final Throwable2Computable<byte[], VcsException, IOException> loader)
            throws VcsException, IOException {
    ...
    public static String getOrLoadAsString(final Project project, FilePath path, VcsRevisionNumber number,  VcsKey vcsKey,
                                            UniqueType type, final Throwable2Computable<byte[], VcsException, IOException> loader)
            throws VcsException, IOException {
    ...
    public static Pair<VcsRevisionNumber, byte[]> getOrLoadCurrentAsBytes(final Project project, FilePath path,  VcsKey vcsKey,
                                                                          final CurrentRevisionProvider loader) throws VcsException, IOException {
        ContentRevisionCache cache = ProjectLevelVcsManager.getInstance(project).getContentRevisionCache();
    ...
    public static Pair<VcsRevisionNumber, String> getOrLoadCurrentAsString(final Project project, FilePath path,  VcsKey vcsKey,
                                                                           final CurrentRevisionProvider loader) throws VcsException, IOException {
