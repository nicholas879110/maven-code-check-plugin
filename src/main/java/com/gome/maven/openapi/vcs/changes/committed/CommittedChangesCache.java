...
import com.gome.maven.openapi.progress.ProgressManagerQueue;
...
import com.gome.maven.openapi.vcs.impl.VcsInitObject;
...
import com.gome.maven.util.MessageBusUtil;
...
public class CommittedChangesCache implements PersistentStateComponent<CommittedChangesCache.State> {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.vcs.changes.committed.CommittedChangesCache");
    ...
    public List<CommittedChangeList> getChanges(ChangeBrowserSettings settings, final VirtualFile file,  final AbstractVcs vcs,
    ...
    public void hasCachesForAnyRoot( final Consumer<Boolean> continuation) {
        myTaskQueue.run(new Runnable() {
            @Override
            public void run() {
    ...
    public void loadIncomingChangesAsync( final Consumer<List<CommittedChangeList>> consumer, final boolean inBackground) {
    ...
    public void processUpdatedFiles(final UpdatedFiles updatedFiles,
                                     final Consumer<List<CommittedChangeList>> incomingChangesConsumer) {
        final Runnable task = new Runnable() {
            @Override
            public void run() {
    ...
    private void pendingUpdateProcessed( Consumer<List<CommittedChangeList>> incomingChangesConsumer) {
    ...
    private void notifyIncomingChangesUpdated( final Collection<CommittedChangeList> receivedChanges) {
    ...
    private void refreshCacheAsync(final ChangesCacheFile cache, final boolean initIfEmpty,
                                    final RefreshResultConsumer consumer) {
        refreshCacheAsync(cache, initIfEmpty, consumer, true);
    }

    private void refreshCacheAsync(final ChangesCacheFile cache, final boolean initIfEmpty,
                                    final RefreshResultConsumer consumer, final boolean asynch) {
        try {
