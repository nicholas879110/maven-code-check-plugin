...
import com.gome.maven.openapi.editor.ex.PrioritizedDocumentListener;
import com.gome.maven.openapi.editor.ex.PrioritizedInternalDocumentListener;
...
public class RangeMarkerTree<T extends RangeMarkerEx> extends IntervalTreeImpl<T> {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.editor.impl.RangeMarkerTree");
    ...
    @Override
    public RMNode<T> addInterval( T interval, int start, int end, boolean greedyToLeft, boolean greedyToRight, int layer) {
    ...
    @Override
    protected RMNode<T> createNewNode( T key, int start, int end, boolean greedyToLeft, boolean greedyToRight, int layer) {
    ...
    @Override
    protected void setNode( T key, IntervalNode<T> intervalNode) {
    ...
    public boolean sweep(final int start, final int end,  final SweepProcessor<T> sweepProcessor) {
        return sweep(new Generator<T>() {
            @Override
            public boolean generateInStartOffsetOrder( Processor<T> processor) {
    ...
    public interface Generator<T> {
        boolean generateInStartOffsetOrder( Processor<T> processor);
    }

    public static <T extends Segment> boolean sweep( Generator<T> generator,  final SweepProcessor<T> sweepProcessor) {
