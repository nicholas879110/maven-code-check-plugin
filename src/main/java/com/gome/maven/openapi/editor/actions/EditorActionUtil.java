...
import com.gome.maven.openapi.actionSystem.ActionPopupMenu;
...
import com.gome.maven.util.EditorPopupHandler;
...
public class EditorActionUtil {
    ...
    public static EditorPopupHandler createEditorPopupHandler( final ActionGroup group) {
        return new EditorPopupHandler() {
            @Override
            public void invokePopup(final EditorMouseEvent event) {
                showEditorPopup(event, group);
    ...
    private static void showEditorPopup(final EditorMouseEvent event,  final ActionGroup group) {
        if (!event.isConsumed() && event.getArea() == EditorMouseEventArea.EDITING_AREA) {
     ...
     */
    public static void makePositionVisible( final Editor editor, final int offset) {
