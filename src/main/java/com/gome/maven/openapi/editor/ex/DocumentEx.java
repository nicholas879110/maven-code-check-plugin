...
public interface DocumentEx extends Document {
    ...
    void registerRangeMarker( RangeMarkerEx rangeMarker,
                             int start,
                             int end,
     ...
     */
    boolean processRangeMarkers( Processor<RangeMarker> processor);

    /**
     * Get range markers which {@link com.gome.maven.openapi.util.TextRange#intersects(int, int)} the specified range
     * and hand them to the {@code processor} in their {@link RangeMarker#getStartOffset()} order
     */
    boolean processRangeMarkersOverlappingWith(int start, int end,  Processor<RangeMarker> processor);
}
