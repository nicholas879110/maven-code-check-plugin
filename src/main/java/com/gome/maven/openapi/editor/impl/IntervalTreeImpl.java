...
abstract class IntervalTreeImpl<T extends MutableInterval> extends RedBlackTree<T> implements IntervalTree<T> {
    ...
    static class IntervalNode<E extends MutableInterval> extends RedBlackTree.Node<E> implements MutableInterval {
        ...
        //  private int modCount; // if it equals to the com.gome.maven.openapi.editor.impl.RedBlackTree.modCount then deltaUpToRoot can be used, otherwise it is expired
        ...
        public IntervalNode( IntervalTreeImpl<E> intervalTree,  E key, int start, int end) {
        ...
        protected Getter<E> createGetter( E interval) {
    ...
    protected abstract IntervalNode<T> createNewNode( T key, int start, int end, boolean greedyToLeft, boolean greedyToRight, int layer);
    protected abstract IntervalNode<T> lookupNode( T key);
    protected abstract void setNode( T key, IntervalNode<T> node);

    private int compareNodes( IntervalNode<T> i1, int delta1,  IntervalNode<T> i2, int delta2,  List<IntervalNode<T>> invalid) {
    ...
    @Override
    public boolean processOverlappingWith(int start, int end,  Processor<? super T> processor) {
        try {
            l.readLock().lock();
            checkMax(true);
    ...
    boolean processOverlappingWithOutside(int start, int end,  Processor<? super T> processor) {
        try {
            l.readLock().lock();
            checkMax(true);
    ...
    @Override
    public boolean processContaining(int offset,  Processor<? super T> processor) {
        try {
            l.readLock().lock();
            checkMax(true);
    ...
    private PeekableIterator<T> overlappingIterator( final TextRangeInterval rangeInterval) {
        l.readLock().lock();
    ...
    private boolean overlaps(IntervalNode<T> root,  TextRangeInterval rangeInterval, int deltaUpToRootExclusive) {
    ...
    private void deleteNodes( List<IntervalNode<T>> collectedAway) {
    ...
    public IntervalTreeImpl.IntervalNode<T> addInterval( T interval, int start, int end, boolean greedyToLeft, boolean greedyToRight, int layer) {
        try {
            l.writeLock().lock();
            checkMax(true);
    ...
    void correctMax( IntervalNode<T> node, int deltaUpToRoot) {
    ...
    static <T extends MutableInterval> PeekableIterator<T> mergingOverlappingIterator( IntervalTreeImpl<T> tree1,
                                                                                       TextRangeInterval tree1Range,
                                                                                       IntervalTreeImpl<T> tree2,
                                                                                       TextRangeInterval tree2Range,
    ...
    static <T extends MutableInterval> PeekableIterator<T> mergeIterators( final PeekableIterator<T> iterator1,
                                                                           final PeekableIterator<T> iterator2,
