...
import com.gome.maven.openapi.editor.impl.softwrap.SoftWrapHelper;
...
import com.gome.maven.util.ui.EmptyClipboardOwner;
...
public class CaretImpl extends UserDataHolderBase implements Caret {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.editor.impl.CaretImpl");
    ...
    private CaretEvent doMoveToLogicalPosition( LogicalPosition pos,
    ...
    void moveToVisualPosition( VisualPosition pos, boolean fireListeners) {
        assertIsDispatchThread();
        validateCallContext();
        if (mySkipChangeRequests) {
            return;
    ...
    CaretEvent moveToLogicalPosition( LogicalPosition pos,
                                     boolean locateBeforeSoftWrap,
                                      StringBuilder debugBuffer,
                                     boolean fireListeners) {
        if (mySkipChangeRequests) {
    ...
    void updateCaretPosition( final DocumentEventImpl event) {
        final DocumentEx document = myEditor.getDocument();
    ...
    private void doSetSelection( final VisualPosition startPosition,
                                final int _startOffset,
                                 final VisualPosition endPosition,
    ...
    void setRangeMarkerEndPosition( VisualPosition endPosition) {
