...
import com.gome.maven.application.options.OptionsConstants;
import com.gome.maven.codeInsight.hint.DocumentFragmentTooltipRenderer;
import com.gome.maven.codeInsight.hint.EditorFragmentComponent;
...
import com.gome.maven.diagnostic.Dumpable;
import com.gome.maven.diagnostic.LogMessageEx;
import com.gome.maven.ide.*;
import com.gome.maven.ide.dnd.DnDManager;
...
import com.gome.maven.openapi.actionSystem.impl.MouseGestureManager;
...
import com.gome.maven.openapi.editor.actionSystem.*;
...
import com.gome.maven.openapi.editor.highlighter.HighlighterClient;
...
import com.gome.maven.openapi.editor.impl.softwrap.SoftWrapDrawingType;
import com.gome.maven.openapi.editor.impl.view.EditorView;
...
import com.gome.maven.openapi.wm.IdeGlassPane;
...
import com.gome.maven.ui.components.JBLayeredPane;
import com.gome.maven.ui.components.JBScrollBar;
...
public final class EditorImpl extends UserDataHolderBase implements EditorEx, HighlighterClient, Queryable, Dumpable {
    ...
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.editor.impl.EditorImpl");
...
//    @MagicConstant(intValues = {VERTICAL_SCROLLBAR_LEFT, VERTICAL_SCROLLBAR_RIGHT})
    ...
    EditorImpl( Document document, boolean viewer,  Project project) {
        ...
        MarkupModelListener markupModelListener = new MarkupModelListener() {
            private boolean areRenderersInvolved( RangeHighlighterEx highlighter) {
        ...
        if (!myUseNewRendering) {
            mySoftWrapModel.addVisualSizeChangeListener(new VisualSizeChangeListener() {
                @Override
                public void onLineWidthsChange(int startLine, int oldEndLine, int newEndLine,  TIntIntHashMap lineWidths) {
    ...
    static Color adjustThumbColor( Color base, boolean dark) {
    ...
    @Override
    public EditorColorsScheme createBoundColorSchemeDelegate( final EditorColorsScheme customGlobalScheme) {
    ...
    */
     ...
     */
    private void setFontSize(final int fontSize,  Point zoomCenter) {
...
//        MacUIUtil.hideCursor();
    ...
    Color getBackgroundColor( final TextAttributes attributes) {
    ...
    private void paintAfterLineEndBackgroundSegments( Graphics g,
    ...
    private int drawSoftWrapAwareBackground( Graphics g,
                                             Color backColor,
                                             Color prevBackColor,
                                             CharSequence text,
                                            int start,
    ...
    private void drawSoftWrap( Graphics g,
                               SoftWrap softWrap,
                               Point position,
                              /*@JdkConstants.FontStyle*/ int fontType,
                               Color backColor,
     ...
     */
    private void paintSelectionOnFirstSoftWrapLineIfNecessary( Graphics g,
                                                               Point position,
                                                               Rectangle clip,
                                                               Color defaultBackground,
                                                              /*@JdkConstants.FontStyle*/ int fontType) {
     ...
     */
    private void paintSelectionOnSecondSoftWrapLineIfNecessary( Graphics g,
                                                                Point position,
                                                                Rectangle clip,
                                                                Color defaultBackground,
                                                               /*@JdkConstants.FontStyle*/ int fontType,
    ...
    private boolean paintPlaceholderText( Graphics g,  Rectangle clip) {
    ...
    private class CachedFontContent {
        ...
        private void addContent( Graphics g, CharSequence _data, int _start, int _end, int _x, int _y,  Color _color, boolean drawWhitespace) {
...
//      myBorderEffect.flushIfCantProlong(g, this, effectType, effectColor);
    ...
    private void drawChars( Graphics g, CharSequence data, int start, int end, int x, int y, boolean drawWhitespace) {
    ...
    private int getTextSegmentWidth( CharSequence text,
                                    int start,
                                    int end,
                                    int xStart,
                                    /*@JdkConstants.FontStyle*/ int fontType,
    ...
    private VisualPosition doLogicalToVisualPosition( LogicalPosition logicalPos, boolean softWrapAware,
    ...
    public LogicalPosition visualToLogicalPosition( VisualPosition visiblePos, boolean softWrapAware) {
...
//    if (myMousePressedInsideSelection) getSelectionModel().removeSelection();
    ...
    private void setSelectionAndBlockActions( MouseEvent mouseDragEvent, VisualPosition startPosition, int startOffset, VisualPosition endPosition, int endOffset) {
    ...
    private void blockActionsIfNeeded( MouseEvent mouseDragEvent,  LogicalPosition startPosition,  LogicalPosition endPosition) {
    ...
    private class CaretCursor {
        ...
        private void paintAt( Graphics g, int x, int y, int width, Caret caret) {
            int lineHeight = getLineHeight();
    ...
    private void resetMouseSelectionState( MouseEvent event) {
        setMouseSelectionState(MOUSE_SELECTION_STATE_NONE);
    ...
    private class MyMouseAdapter extends MouseAdapter {
        ...
        private boolean processMousePressed( final MouseEvent e) {
    ...
    private static class MyTransferHandler extends TransferHandler {
        private static EditorImpl getEditor( JComponent comp) {
    ...
    private class MyScrollPane extends JBScrollPane {
        ...
        protected boolean isOverlaidScrollbar( JScrollBar scrollbar) {
