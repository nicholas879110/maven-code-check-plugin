...
import com.gome.maven.openapi.editor.ex.ScrollingModelEx;
import com.gome.maven.openapi.editor.ex.util.EditorUtil;
import com.gome.maven.openapi.editor.impl.*;
...
public class SoftWrapApplianceManager implements Dumpable {
    ...
    public SoftWrapApplianceManager( SoftWrapsStorage storage,
                                     EditorEx editor,
    ...
    @SuppressWarnings({"ForLoopReplaceableByForEach"})
    private void notifyListenersOnFoldRegion( FoldRegion foldRegion, int collapsedFoldingWidthInColumns, int visualLine) {
        for (int i = 0; i < myListeners.size(); i++) {
            // Avoid unnecessary Iterator object construction as this method is expected to be called frequently.
            SoftWrapAwareDocumentParsingListener listener = myListeners.get(i);
    ...
    @SuppressWarnings({"ForLoopReplaceableByForEach"})
    private void notifyListenersOnVisualLineStart( EditorPosition position) {
        for (int i = 0; i < myListeners.size(); i++) {
            // Avoid unnecessary Iterator object construction as this method is expected to be called frequently.
            SoftWrapAwareDocumentParsingListener listener = myListeners.get(i);
