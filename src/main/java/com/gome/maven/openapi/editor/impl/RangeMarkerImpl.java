...
public class RangeMarkerImpl extends UserDataHolderBase implements RangeMarkerEx, MutableInterval {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.editor.impl.RangeMarkerImpl");
    ...
    protected RangeMarkerImpl( DocumentEx document, int start, int end, boolean register) {
        this(document, start, end, register, false, false);
    }
    private RangeMarkerImpl( DocumentEx document, int start, int end, boolean register, boolean greedyToLeft, boolean greedyToRight) {
        if (start < 0) {
    ...
    public void invalidate( final Object reason) {
        setValid(false);
