...
class VisualToLogicalCalculationStrategy extends AbstractMappingStrategy<LogicalPosition> {
    ...
    VisualToLogicalCalculationStrategy( EditorEx editor,  SoftWrapsStorage storage,  List<CacheEntry> cache)
    {
        super(editor, storage, cache);
        mySearchKey = new CacheEntry(0, editor);
    }

    public void init( final VisualPosition targetVisual,  final List<CacheEntry> cache) {
        reset();
    ...
    @Override
    protected LogicalPosition buildIfExceeds( EditorPosition context,  FoldRegion foldRegion) {
