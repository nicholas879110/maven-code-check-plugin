...
public class ZipFileCache {
    ...
    private static class CacheRecord {
        ...
        private CacheRecord( String path,  ZipFile file) throws IOException {
            this.path = path;
            this.file = file;
        }
    }
    ...
    public static ZipFile acquire( String path) throws IOException {
    ...
    private static int tryCloseFiles() {
        ...
        close(toClose);
        Logger.getInstance("").warn("too many open files, closed: " + toClose.size());
        return toClose.size();
    }
    ...
    public static void release( ZipFile file) {
        synchronized (ourLock) {
    ...
    private static void close( List<ZipFile> files) {
