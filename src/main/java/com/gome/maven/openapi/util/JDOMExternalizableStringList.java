...
public class JDOMExternalizableStringList extends ArrayList<String> implements JDOMExternalizable {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.openapi.util.JDOMExternalizableStringList");
    ...
    public void readExternal(Element element) throws InvalidDataException {
        ...
        for (Element listElement : (List<Element>)element.getChildren(ATTR_LIST)) {
            if (callerClass == null) {
                callerClass = ReflectionUtil.findCallerClass(2);
                assert callerClass != null;
            }
            final ClassLoader classLoader = callerClass.getClassLoader();
            for (Element listItemElement : (List<Element>)listElement.getChildren(ATTR_ITEM)) {
