...
class ComponentManagerConfigurator {
    ...
    private void loadConfiguration( ComponentConfig[] configs, final boolean defaultProject, final PluginDescriptor descriptor) {
        for (ComponentConfig config : configs) {
            loadSingleConfig(defaultProject, config, descriptor);
        }
    }

    private void loadSingleConfig(final boolean defaultProject,  ComponentConfig config, final PluginDescriptor descriptor) {
