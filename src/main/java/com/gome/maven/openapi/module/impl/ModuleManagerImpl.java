...
import com.gome.maven.util.containers.hash.EqualityPolicy;
...
public abstract class ModuleManagerImpl extends ModuleManager implements ProjectComponent, PersistentStateComponent<Element> {
    ...
    public void removeFailedModulePath( ModulePath modulePath) {
        myFailedModulePaths.remove(modulePath);
    }
    ...
    public Module newModule( String filePath, final String moduleTypeId) {
    ...
    public Module loadModule( String filePath) throws InvalidDataException, IOException, JDOMException, ModuleWithNameAlreadyExists {
        incModificationCount();
        final ModifiableModuleModel modifiableModel = getModifiableModel();
    ...
    class ModuleModelImpl implements ModifiableModuleModel {
        ...
        public Module loadModule( String filePath) throws InvalidDataException, IOException, ModuleWithNameAlreadyExists {
            assertWritable();
            try {
    ...
    void fireModuleRenamedByVfsEvent( final Module module,  final String oldName) {
        ProjectRootManagerEx.getInstanceEx(myProject).makeRootsChange(new Runnable() {
            @Override
            public void run() {
