...
import com.gome.maven.codeInsight.lookup.Classifier;
...
import com.gome.maven.util.containers.FlatteningIterator;
...
public class StatisticsWeigher extends CompletionWeigher {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.codeInsight.completion.StatisticsWeigher.LookupStatisticsWeigher");
    private static final Key<StatisticsInfo> BASE_STATISTICS_INFO = Key.create("Base statistics info");

    @Override
    public Comparable weigh( final LookupElement item,  final CompletionLocation location) {
    ...
    public static class LookupStatisticsWeigher extends Classifier<LookupElement> {
        ...
        private static int weigh( LookupElement item, final StatisticsInfo baseInfo, WeighingContext context) {
            if (baseInfo == StatisticsInfo.EMPTY) {
                return 0;
            }
