...
import com.gome.maven.codeInsight.daemon.impl.quickfix.QuickFixActionRegistrarImpl;
...
import com.gome.maven.psi.search.searches.SuperMethodsSearch;
...
public class GenericsHighlightUtil {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.codeInsight.daemon.impl.analysis.GenericsHighlightUtil");
    ...
    public static HighlightInfo checkInferredTypeArguments(PsiTypeParameter[] typeParameters,
        ...
        if (inferredTypeArgument != null) {
            ...
            PsiClass boundClass = extendsType instanceof PsiClassType ? ((PsiClassType)extendsType).resolve() : null;
             ...
             String messageKey = boundClass == null || typeParameter.isInterface() == boundClass.isInterface()
        ...
        for (PsiClassType type1 : bounds) {
            ...
            if (!bound.equalsToText(CommonClassNames.JAVA_LANG_OBJECT) && GenericsUtil.checkNotInBounds(type, bound, referenceParameterList)) {
                PsiClass boundClass = bound instanceof PsiClassType ? ((PsiClassType)bound).resolve() : null;
                 ...
                 final String messageKey = boundClass == null || referenceClass == null || referenceClass.isInterface() == boundClass.isInterface()
    ...
    public static HighlightInfo checkElementInTypeParameterExtendsList( PsiReferenceList referenceList,
                                                                        PsiClass aClass,
    ...
    public static Collection<HighlightInfo> checkOverrideEquivalentMethods( LanguageLevel languageLevel,
                                                                            PsiClass aClass) {
    ...
    static HighlightInfo checkDefaultMethodOverrideEquivalentToObjectNonPrivate( LanguageLevel languageLevel,
                                                                                 PsiClass aClass,
                                                                                 PsiMethod method,
    ...
    static HighlightInfo checkUnrelatedDefaultMethods( PsiClass aClass,
    ...
    private static HighlightInfo checkSameErasureNotSubSignatureInner( HierarchicalMethodSignature signature,
                                                                       PsiManager manager,
                                                                       PsiClass aClass,
                                                                       Map<MethodSignature, MethodSignatureBackedByPsiMethod> sameErasureMethods) {
    ...
    private static HighlightInfo getSameErasureMessage(final boolean sameClass,  PsiMethod method,  PsiMethod superMethod,
                                                       TextRange textRange) {
         final String key = sameClass ? "generics.methods.have.same.erasure" :
    ...
    public static HighlightInfo checkTypeParametersList(PsiTypeParameterList list, PsiTypeParameter[] parameters,  LanguageLevel level) {
     ...
     */
    static HighlightInfo checkRawOnParameterizedType( PsiJavaCodeReferenceElement parent, PsiElement resolved) {
    ...
    public static HighlightInfo areSupersAccessible( PsiClass aClass) {
        return areSupersAccessible(aClass, aClass.getResolveScope(), HighlightNamesUtil.getClassDeclarationTextRange(aClass));
    }

    public static HighlightInfo areSupersAccessible( PsiClass aClass, PsiElement ref) {
        return areSupersAccessible(aClass, ref.getResolveScope(), ref.getTextRange());
    }

    private static HighlightInfo areSupersAccessible( PsiClass aClass,
