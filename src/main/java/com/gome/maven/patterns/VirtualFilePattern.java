...
public class VirtualFilePattern extends TreeElementPattern<VirtualFile, VirtualFile, VirtualFilePattern> {
    ...
    public VirtualFilePattern ofType(final FileType type) {
        return with(new PatternCondition<VirtualFile>("ofType") {
            @Override
            public boolean accepts( final VirtualFile virtualFile, final ProcessingContext context) {
    ...
    @Override
    protected VirtualFile getParent( final VirtualFile t) {
        return t.getParent();
    }

    @Override
    protected VirtualFile[] getChildren( final VirtualFile file) {
