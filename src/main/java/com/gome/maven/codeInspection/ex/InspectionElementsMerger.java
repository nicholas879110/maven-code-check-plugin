...
public abstract class InspectionElementsMerger {
    public static final ExtensionPointName<InspectionElementsMerger> EP_NAME = ExtensionPointName.create("com.gome.maven.inspectionElementsMerger");
    ...
    protected static void collectContent(Element sourceElement, List<Element> options, Map<String, Element> scopes) {
        if (sourceElement != null) {
            for (Element element :(List<Element>) sourceElement.getChildren()) {
                if ("scope".equals(element.getName())) {
                    ...
                    if (scopes.containsKey(scopeName)) {
                        ...
                        for (Element scopeEl : (List<Element>)element.getChildren()) {
                            scopeElement.addContent((Element)scopeEl.clone());
                        }
                    } else {
                        scopes.put(scopeName, (Element) element.clone());
                    }
                    continue;
                }
                options.add((Element)element.clone());
            }
        }
    }
}
