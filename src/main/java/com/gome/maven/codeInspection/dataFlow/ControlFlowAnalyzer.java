...
import com.gome.maven.codeInspection.dataFlow.value.*;
...
import static com.gome.maven.psi.CommonClassNames.*;

public class ControlFlowAnalyzer extends JavaElementVisitor {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.codeInspection.dataFlow.ControlFlowAnalyzer");
    public static final String ORG_JETBRAINS_ANNOTATIONS_CONTRACT = null;//自己修改的Contract.class.getName();
    ...
    public ControlFlow buildControlFlow( PsiElement codeFragment, boolean ignoreAssertions) {
    ...
    private void returnCheckingFinally(boolean viaException,  PsiElement anchor) {
    ...
    private void addThrowCode( CatchDescriptor cd,  PsiElement explicitThrower) {
        if (cd == null) {
    ...
    private void acceptBinaryRightOperand( IElementType op, PsiType type,
    ...
    private void addMethodThrows(PsiMethod method,  PsiElement explicitCall) {
        CatchDescriptor cd = findNextCatch(false);
        if (method != null) {
    ...
    private void initException(PsiType ref,  CatchDescriptor cd) {
        if (cd == null) return;
        addInstruction(new PushInstruction(getExceptionHolder(cd), null));
    ...
    private static List<MethodContract> getMethodCallContracts( final PsiMethod method,  PsiMethodCallExpression call) {
    ...
    static List<MethodContract> getMethodContracts( final PsiMethod method) {
