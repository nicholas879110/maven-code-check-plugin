...
import com.gome.maven.util.containers.LimitedPool;
...
public class PsiBuilderImpl extends UserDataHolderBase implements PsiBuilder {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.lang.impl.PsiBuilderImpl");
    ...
    public PsiBuilderImpl( Project project,
                          ...
                          CharTable charTable,
                           ...
                           MyTreeStructure parentLightTree) {
    ...
    public PsiBuilderImpl( final Project project,
    ...
    private static class DoneWithErrorMarker extends DoneMarker {
        ...
        private DoneWithErrorMarker( StartMarker marker, final int currentLexeme, final String message) {
    ...
    private void errorBefore( Marker marker, String message,  Marker before) {
    ...
    private void doValidityChecks( Marker marker,  final Marker before) {
        final DoneMarker doneMarker = ((StartMarker)marker).myDoneMarker;
        if (doneMarker != null) {
    ...
    private DiffLog merge( final ASTNode oldRoot,  StartMarker newRoot) {
    ...
    private void assertMarkersBalanced(boolean condition,  ProductionMarker marker) {
        if (condition) return;
    ...
    private static class MyTreeStructure implements FlyweightCapableTreeStructure<LighterASTNode> {
        ...
        public MyTreeStructure( StartMarker root,  final MyTreeStructure parentTree) {
            if (parentTree == null) {
        ...
        public LighterASTNode prepareForGetChildren( final LighterASTNode node) {
            return node;
        }
    ...
    private TreeElement createLeaf( IElementType type, final int start, final int end) {
