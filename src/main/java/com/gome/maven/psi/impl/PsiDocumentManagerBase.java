...
import com.gome.maven.psi.impl.smartPointers.SmartPointerManagerImpl;
...
public abstract class PsiDocumentManagerBase extends PsiDocumentManager implements DocumentListener, DocumentBulkUpdateListener {
    protected static final Logger LOG = Logger.getInstance("#com.gome.maven.psi.impl.PsiDocumentManagerImpl");
    ...
    public PsiDocumentManagerBase( final Project project,
                                   PsiManager psiManager,
                                   SmartPointerManager smartPointerManager,
                                   MessageBus bus,
                                    final DocumentCommitProcessor documentCommitProcessor) {
        myProject = project;
        ...
        bus.connect().subscribe(PsiDocumentTransactionListener.TOPIC, new PsiDocumentTransactionListener() {
            @Override
            public void transactionStarted( Document document,  PsiFile file) {
    ...
    public static void cachePsi( Document document,  PsiFile file) {
    ...
    private FileViewProvider getCachedViewProvider( VirtualFile virtualFile) {
    ...
    @Override
    public void performForCommittedDocument( final Document doc,  final Runnable action) {
     ...
     */
    public boolean cancelAndRunWhenAllCommitted(  Object key,  final Runnable action) {
        ApplicationManager.getApplication().assertIsDispatchThread();
    ...
    // public for Upsource
    public boolean finishCommit( final Document document,
                                 final List<Processor<Document>> finishProcessors,
    ...
    protected boolean finishCommitInWriteAction( final Document document,
                                                 final List<Processor<Document>> finishProcessors,
                                                final boolean synchronously) {
    ...
    protected void doCommit( final Document document) {
     ...
     */
    @Override
    public boolean performWhenAllCommitted( final Runnable action) {
        ApplicationManager.getApplication().assertIsDispatchThread();
    ...
    boolean isInUncommittedSet( Document document) {
