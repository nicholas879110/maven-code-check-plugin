...
import com.gome.maven.execution.runners.GenericProgramRunner;
import com.gome.maven.execution.ui.layout.impl.DockableGridContainerFactory;
...
import com.gome.maven.openapi.wm.ex.ToolWindowManagerAdapter;
...
public class RunContentManagerImpl implements RunContentManager, Disposable {
    ...
    private void registerToolwindow( final Executor executor,  ToolWindowManagerEx toolWindowManager) {
    ...
    @Override
    public void hideRunContent( final Executor executor, final RunContentDescriptor descriptor) {
        ApplicationManager.getApplication().invokeLater(new Runnable() {
            @Override
            public void run() {
    ...
    @Override
    public boolean removeRunContent( final Executor executor, final RunContentDescriptor descriptor) {
    ...
    public void showRunContent( final Executor executor,  final RunContentDescriptor descriptor, final long executionId) {
    ...
    @Deprecated
    public RunContentDescriptor getReuseContent(Executor requestor,  RunContentDescriptor contentToReuse) {
        if (ApplicationManager.getApplication().isUnitTestMode()) {
            return null;
        }
    ...
    @Override
    public RunContentDescriptor getReuseContent(Executor requestor,  ExecutionEnvironment executionEnvironment) {
    ...
    @Override
    public void showRunContent( Executor info,  RunContentDescriptor descriptor,  RunContentDescriptor contentToReuse) {
        copyContentAndBehavior(descriptor, contentToReuse);
        showRunContent(info, descriptor, descriptor.getExecutionId());
    }

    public static void copyContentAndBehavior( RunContentDescriptor descriptor,  RunContentDescriptor contentToReuse) {
        if (contentToReuse != null) {
    ...
    private static Content getContentFromManager(ContentManager contentManager,  String preferredName, long executionId) {
    ...
    private static RunContentDescriptor getRunContentDescriptorByContent( Content content) {
    ...
    public void addRunContentListener( final RunContentListener listener, final Executor executor) {
        ...
        myProject.getMessageBus().connect(disposable).subscribe(TOPIC, new RunContentWithExecutorListener() {
            @Override
            public void contentSelected( RunContentDescriptor descriptor,  Executor executor2) {
            ...
            @Override
            public void contentRemoved( RunContentDescriptor descriptor,  Executor executor2) {
                if (executor2.equals(executor)) {
    ...
    private class CloseListener extends ContentManagerAdapter implements ProjectManagerListener {
        ...
        private CloseListener( final Content content,  Executor executor) {
            myContent = content;
