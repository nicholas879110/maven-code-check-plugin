...
public class RunnerAndConfigurationSettingsImpl implements JDOMExternalizable, Cloneable, RunnerAndConfigurationSettings, Comparable {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.execution.impl.RunnerAndConfigurationSettings");
    ...
    private final RunnerItem<RunnerSettings> myRunnerSettings = new RunnerItem<RunnerSettings>("RunnerSettings") {
        @Override
        protected RunnerSettings createSettings( ProgramRunner runner) {
    ...
    private final RunnerItem<ConfigurationPerRunnerSettings> myConfigurationPerRunnerSettings = new RunnerItem<ConfigurationPerRunnerSettings>("ConfigurationWrapper") {
        @Override
        protected ConfigurationPerRunnerSettings createSettings( ProgramRunner runner) {
    ...
    private abstract class RunnerItem<T> {
        ...
        public void getState( Element element) throws WriteExternalException {
            ...
            if (unloadedSettings != null) {
                for (Element unloadedSetting : unloadedSettings) {
                    runnerSettings.add((Element) unloadedSetting.clone());
                }
            }
        ...
        protected abstract T createSettings( ProgramRunner runner);

        private void add( Element state,  ProgramRunner runner,  T data) throws InvalidDataException {
            if (runner == null) {
                if (unloadedSettings == null) {
        ...
        public T getOrCreateSettings( ProgramRunner runner) {
    ...
    @Override
    public void checkSettings( Executor executor) throws RuntimeConfigurationException {
    ...
    @SuppressWarnings("deprecation")
    private <T> void importFromTemplate( RunnerItem<T> templateItem,  RunnerItem<T> item) {
