...
public class MapReduceIndex<Key, Value, Input> implements UpdatableIndex<Key,Value, Input> {
    ...
    public MapReduceIndex( final ID<Key, Value> indexId,
    ...
    private static void cleanMapping( PersistentHashMap<?, ?> index) {
    ...
    @Override
    public final Computable<Boolean> update(final int inputId,  Input content) {
    ...
    private static final com.gome.maven.openapi.util.Key<Integer> ourSavedContentHashIdKey = com.gome.maven.openapi.util.Key.create("saved.content.hash.id");
    private static final com.gome.maven.openapi.util.Key<Integer> ourSavedUncommittedHashIdKey = com.gome.maven.openapi.util.Key.create("saved.uncommitted.hash.id");
    ...
    public class SimpleUpdateData extends UpdateData<Key, Value> {
        ...
        public SimpleUpdateData(ID<Key,Value> indexId, int id,  Map<Key, Value> data,  NotNullComputable<Collection<Key>> getter) {
            super(indexId);
            savedInputId = id;
            newData = data;
    ...
    protected void updateWithMap(final int inputId,
                                  UpdateData<Key, Value> updateData) throws StorageException {
        getWriteLock().lock();
        try {
            try {
                ValueContainerImpl.ourDebugIndexInfo.set(myIndexId);
