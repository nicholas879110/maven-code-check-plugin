...
import com.gome.maven.openapi.wm.IdeFrame;
import com.gome.maven.util.concurrency.QueueProcessor;
...
public class Alarm implements Disposable {
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.util.Alarm");
    ...
    public enum ThreadToUse {
         ...
         *
         * @see com.gome.maven.openapi.application.Application#executeOnPooledThread(java.util.concurrent.Callable)
         */
        POOLED_THREAD,
    ...
    public void addRequest( Runnable request, int delayMillis,  final ModalityState modalityState) {
        LOG.assertTrue(myThreadToUse == ThreadToUse.SWING_THREAD);
        _addRequest(request, delayMillis, modalityState);
    }

    public void addRequest( Runnable request, long delayMillis,  final ModalityState modalityState) {
        LOG.assertTrue(myThreadToUse == ThreadToUse.SWING_THREAD);
        _addRequest(request, delayMillis, modalityState);
    }
    ...
    private class Request implements Runnable {
        ...
        private Request( final Runnable task,  ModalityState modalityState, long delayMillis) {
            synchronized (LOCK) {
                myTask = task;
    ...
    public Alarm setActivationComponent( final JComponent component) {
