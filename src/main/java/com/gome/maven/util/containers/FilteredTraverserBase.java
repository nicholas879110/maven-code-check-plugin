...
import static com.gome.maven.openapi.util.Conditions.not;
...
public abstract class FilteredTraverserBase<T, Self extends FilteredTraverserBase<T, Self>> implements Iterable<T> {
    ...
    protected FilteredTraverserBase( Meta<T> meta, Function<T, ? extends Iterable<? extends T>> tree) {
        this.tree = tree;
    ...
    public Self expand( Condition<? super T> c) {
        return newInstance(meta.expand(c));
    }

    
    public Self regard( Condition<? super T> c) {
    ...
    public Self filter( Condition<? super T> c) {
        return newInstance(meta.filter(c));
    }

    
    public <C> JBIterable<C> filter( Class<C> type) {
        return traverse().filter(type);
    }

    
    public Self forceIgnore( Condition<? super T> c) {
        return newInstance(meta.forceIgnore(c));
    }

    
    public Self forceDisregard( Condition<? super T> c) {
        return newInstance(meta.forceDisregard(c));
    }

    
    public JBIterable<T> children( T node) {
    ...
    protected static class Meta<T> {
        ...
        public Meta( Iterable<? extends T> roots,
