...
import static com.gome.maven.util.xmlb.Constants.*;
...
class MapBinding extends Binding implements MultiNodeBinding, MainBinding {
    ...
    private void serializeKeyOrValue( Element entry,  String attributeName,  Object value,  Binding binding,  SerializationFilter filter) {
        if (value == null) {
            return;
        }
    ...
    private Object deserializeKeyOrValue( Element entry,  String attributeName, Object context,  Binding binding,  Class<?> valueClass) {
        ...
        if (attribute != null) {
            ...
            assert binding != null;
            for (Element element : (List<Element>)entry.getChildren()) {
