 ...
 */
package com.gome.maven.util.proxy;
...
public class CommonProxy extends ProxySelector {
    ...
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.util.proxy.CommonProxy");
    ...
    public static void isInstalledAssertion() {
        ...
        if (ourInstance != aDefault) {
            ...
            if (ourWrong != aDefault || itsTime()) {
                LOG.error("ProxySelector.setDefault() was changed to [" + aDefault.toString() + "] - other than com.gome.maven.util.proxy.CommonProxy.ourInstance.\n" +
                        "This will make some " + ApplicationNamesInfo.getInstance().getProductName() + " network calls fail.\n" +
                        "Instead, methods of com.gome.maven.util.proxy.CommonProxy should be used for proxying.");
                ourWrong = aDefault;
            }
            ProxySelector.setDefault(ourInstance);
    ...
    public void noProxy( final String protocol,  final String host, final int port) {
        synchronized (myLock) {
    ...
    public void removeNoProxy( final String protocol,  final String host, final int port) {
        synchronized (myLock) {
    ...
    public void noAuthentication( final String protocol,  final String host, final int port) {
        synchronized (myLock) {
            LOG.debug("no proxy added: " + protocol + "://" + host + ":" + port);
            myNoProxy.add(Pair.create(new HostInfo(protocol, host, port), Thread.currentThread()));
        }
    ...
    public void removeNoAuthentication( final String protocol,  final String host, final int port) {
        synchronized (myLock) {
            LOG.debug("no proxy removed: " + protocol + "://" + host + ":" + port);
            myNoProxy.remove(Pair.create(new HostInfo(protocol, host, port), Thread.currentThread()));
        }
    ...
    public void setCustom( final String key,  final ProxySelector proxySelector) {
        synchronized (myLock) {
    ...
    public void setCustomAuth( final String key, final NonStaticAuthenticator authenticator) {
        synchronized (myLock) {
    ...
    public void removeCustomAuth( final String key) {
        synchronized (myLock) {
    ...
    public void removeCustom( final String key) {
        synchronized (myLock) {
