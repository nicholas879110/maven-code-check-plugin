...
import com.gome.maven.openapi.util.ThreadLocalCachedValue;
...
import com.gome.maven.util.containers.SLRUCache;
...
public class PersistentHashMap<Key, Value> extends PersistentEnumeratorDelegate<Key> implements PersistentMap<Key, Value> {
    ...
    private static final Logger LOG = Logger.getInstance("#com.gome.maven.util.io.PersistentHashMap");
    ...
    private final LimitedPool<BufferExposingByteArrayOutputStream> myStreamPool = new LimitedPool<BufferExposingByteArrayOutputStream>(10, new LimitedPool.ObjectFactory<BufferExposingByteArrayOutputStream>() {
        ...
        @Override
        public void cleanup( final BufferExposingByteArrayOutputStream appendStream) {
            appendStream.reset();
        }
    });
    ...
    private SLRUCache<Key, BufferExposingByteArrayOutputStream> createAppendCache(final KeyDescriptor<Key> keyDescriptor) {
        return new SLRUCache<Key, BufferExposingByteArrayOutputStream>(16 * 1024, 4 * 1024, keyDescriptor) {
            ...
            @Override
            protected void onDropFromCache(final Key key,  final BufferExposingByteArrayOutputStream bytes) {
     ...
     * {@link #processKeysWithExistingMapping(com.gome.maven.util.Processor)} to process only keys with existing mappings
     */
    @Override
    ...
    private int updateValueId(int keyId, long value, long oldValue,  Key key, int processingKey) throws IOException {
        if (myDirectlyStoreLongFileOffsetMode) {
