...
import com.gome.maven.ide.actions.GotoFileAction;
import com.gome.maven.ide.actions.WindowAction;
...
import com.gome.maven.openapi.fileTypes.ex.FileTypeManagerEx;
...
import com.gome.maven.ui.popup.PopupOwner;
...
public abstract class ChooseByNameBase {
    ...
    private void setNamesSync(boolean checkboxState,  String[] value) {
    ...
    private void setElementsToList(int pos,  Collection<?> elements) {
        myListUpdater.cancelAll();
        if (checkDisposed()) return;
        if (elements.isEmpty()) {
            myListModel.clear();
    ...
    private class ListUpdater {
        ...
        public void appendToModel( List<ModelDiff.Cmd> commands, final int selectionPos) {
            myAlarm.cancelAllRequests();
    ...
    private abstract class ShowFindUsagesAction extends AnAction {
        ...
        private void showUsageView( List<PsiElement> targets,
                                    List<Usage> usages,
