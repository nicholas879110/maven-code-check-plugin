...
import com.gome.maven.openapi.progress.Progressive;
...
public class AbstractTreeBuilder implements Disposable {
    ...
    public final void select(final Object element,  final Runnable onDone) {
        if (isDisposed()) return;

        getUi().userSelect(new Object[]{element}, new UserRunnable(onDone), false, true);
    }

    public final void select(final Object element,  final Runnable onDone, boolean addToSelection) {
        if (isDisposed()) return;

        getUi().userSelect(new Object[]{element}, new UserRunnable(onDone), addToSelection, true);
    }

    public final void select(final Object[] elements,  final Runnable onDone) {
        if (isDisposed()) return;

        getUi().userSelect(elements, new UserRunnable(onDone), false, true);
    }

    public final void select(final Object[] elements,  final Runnable onDone, boolean addToSelection) {
        if (isDisposed()) return;
