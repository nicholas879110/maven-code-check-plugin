...
import com.gome.maven.ide.projectView.impl.ProjectViewTree;
import com.gome.maven.ide.scopeView.nodes.BasePsiNode;
...
public class ScopeTreeViewPanel extends JPanel implements Disposable {
    private static final Logger LOG = Logger.getInstance("com.gome.maven.ide.scopeView.ScopeTreeViewPanel");
    ...
    public void refreshScope( NamedScope scope) {
        FileTreeModelBuilder.clearCaches(myProject);
    ...
    private class MyChangesListListener extends ChangeListAdapter {
        ...
        private void fireListeners(ChangeList list,  String oldName) {
            AbstractProjectViewPane pane = ProjectView.getInstance(myProject).getCurrentProjectViewPane();
            if (pane == null || !ScopeViewPane.ID.equals(pane.getId())) {
                return;
            }
