...
class DevelopersLoader {
    private static final String DEVELOPERS_LIST_URL = "http://ea-engine.labs.gome.maven.net/data?category=developers";
    public static final int TIMEOUT = 1000;

    private DevelopersLoader() {
    }

    public static Collection<Developer> fetchDevelopers( final ProgressIndicator indicator) throws IOException {
        return HttpRequests.request(DEVELOPERS_LIST_URL).connect(new HttpRequests.RequestProcessor<Collection<Developer>>() {
            @Override
            public Collection<Developer> process( HttpRequests.Request request) throws IOException {
