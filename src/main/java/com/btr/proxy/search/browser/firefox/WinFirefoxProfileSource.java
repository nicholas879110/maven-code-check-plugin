...
class WinFirefoxProfileSource implements FirefoxProfileSource {
    ...
    public File getProfileFolder() throws IOException {
        ...
        if (!cfgDir.exists()) {
            Logger.log(this.getClass(), LogLevel.DEBUG, "Firefox windows settings folder not found.", new Object[0]);
            return null;
        } else {
            ...
            if (profiles != null && profiles.length != 0) {
                ...
                for(int i$ = 0; i$ < len$; ++i$) {
                    ...
                    if (p.getName().endsWith(".default")) {
                        Logger.log(this.getClass(), LogLevel.TRACE, "Firefox windows settings folder is {0}.", new Object[]{p});
                        return p;
                    }
                }

                Logger.log(this.getClass(), LogLevel.TRACE, "Firefox windows settings folder is {0}.", new Object[]{profiles[0]});
                return profiles[0];
            } else {
                Logger.log(this.getClass(), LogLevel.DEBUG, "Firefox windows settings folder not found.", new Object[0]);
                return null;
