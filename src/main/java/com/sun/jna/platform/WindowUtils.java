...
public class WindowUtils {
    ...
    private static class X11WindowUtils extends WindowUtils.NativeWindowUtils {
        ...
        private static Pixmap createBitmap(Display dpy, com.sun.jna.platform.unix.X11.Window win, Raster raster) {
            X11 x11 = X11.INSTANCE;
            Rectangle bounds = raster.getBounds();
            int width = bounds.x + bounds.width;
            int height = bounds.y + bounds.height;
            Pixmap pm = x11.XCreatePixmap(dpy, win, width, height, 1);
            GC gc = x11.XCreateGC(dpy, pm, new NativeLong(0L), (XGCValues)null);
            if (gc == null) {
                return null;
            } else {
                ...
                try {
                    RasterRangesUtils.outputOccupiedRanges(raster, new RangesOutput() {
                        public boolean outputRange(int x, int y, int w, int h) {
                            rlist.add(new Rectangle(x, y, w, h));
                            return true;
                        }
                    });
                    XRectangle[] rects = (XRectangle[])((XRectangle[])(new XRectangle()).toArray(rlist.size()));
        ...
        private synchronized long[] getAlphaVisualIDs() {
            if (this.didCheck) {
                ...
                this.didCheck = true;
                X11 x11 = X11.INSTANCE;
                Display dpy = x11.XOpenDisplay((String)null);
                if (dpy == null) {
                    return this.alphaVisualIDs;
                } else {
                    ...
                    try {
                        ...
                        List<VisualID> list = new ArrayList();
                        XVisualInfo[] infos = (XVisualInfo[])((XVisualInfo[])info.toArray(pcount.getValue()));

                        int i;
                        for(i = 0; i < infos.length; ++i) {
                            XRenderPictFormat format = Xrender.INSTANCE.XRenderFindVisualFormat(dpy, infos[i].visual);
        ...
        private static com.sun.jna.platform.unix.X11.Window getContentWindow(Window w, Display dpy, com.sun.jna.platform.unix.X11.Window win, Point offset) {
            if (w instanceof Frame && !((Frame)w).isUndecorated() || w instanceof Dialog && !((Dialog)w).isUndecorated()) {
                X11 x11 = X11.INSTANCE;
                WindowByReference rootp = new WindowByReference();
                WindowByReference parentp = new WindowByReference();
                ...
                if (i$ < len$) {
                    int id = ids[i$];
                    com.sun.jna.platform.unix.X11.Window child = new com.sun.jna.platform.unix.X11.Window((long)id);
        ...
        private static com.sun.jna.platform.unix.X11.Window getDrawable(Component w) {
            int id = (int)Native.getComponentID(w);
            return id == 0 ? null : new com.sun.jna.platform.unix.X11.Window((long)id);
        }
            ...
            if (!this.isWindowAlphaSupported()) {
                ...
                Runnable action = new Runnable() {
                    public void run() {
                        ...
                        if (dpy != null) {
                            try {
                                com.sun.jna.platform.unix.X11.Window win = WindowUtils.X11WindowUtils.getDrawable(w);
                                if (alpha == 1.0F) {
                ...
                public void run() {
                    ...
                    if (dpy != null) {
                        ...
                        try {
                            ...
                            ext.XShapeCombineMask(dpy, win, 0, 0, 0, pm == null ? Pixmap.None : pm, 0);
                        } finally {
                            if (pm != null) {
        ...
        protected void setMask(Component w, final Raster raster) {
            this.setWindowShape(this.getWindow(w), new WindowUtils.X11WindowUtils.PixmapSource() {
                public Pixmap getPixmap(Display dpy, com.sun.jna.platform.unix.X11.Window win) {
        ...
        private interface PixmapSource {
            Pixmap getPixmap(Display var1, com.sun.jna.platform.unix.X11.Window var2);
        }
        ...
        private class X11TransparentContentPane extends WindowUtils.NativeWindowUtils.TransparentContentPane {
            ...
            protected void paintDirect(BufferedImage buf, Rectangle bounds) {
                ...
                X11 x11 = X11.INSTANCE;
                Display dpy = x11.XOpenDisplay((String)null);
                com.sun.jna.platform.unix.X11.Window win = WindowUtils.X11WindowUtils.getDrawable(window);
                Point offset = new Point();
                win = WindowUtils.X11WindowUtils.getContentWindow(window, dpy, win, offset);
                GC gc = x11.XCreateGC(dpy, win, new NativeLong(0L), (XGCValues)null);
                ...
                XImage image = x11.XCreateImage(dpy, xwa.visual, 32, 2, 0, this.buffer, w, h, 32, w * 4);
    ...
    private static class W32WindowUtils extends WindowUtils.NativeWindowUtils {
        ...
        public void setWindowAlpha(final Window w, final float alpha) {
            if (!this.isWindowAlphaSupported()) {
                ...
                this.whenDisplayable(w, new Runnable() {
                    public void run() {
                        HWND hWnd = W32WindowUtils.this.getHWnd(w);
                        ...
                        if (W32WindowUtils.this.usingUpdateLayeredWindow(w)) {
                            ...
                            blend.AlphaFormat = 1;
                            user.UpdateLayeredWindow(hWnd, (HDC)null, (POINT)null, (SIZE)null, (HDC)null, (POINT)null, 0, blend, 2);
                        } else if (alpha == 1.0F) {
                            flags &= -524289;
        ...
        private void setMask(Component w, Area area) {
            ...
            POINT[] lppt = (POINT[])((POINT[])(new POINT()).toArray(points.size()));
        ...
        private class W32TransparentContentPane extends WindowUtils.NativeWindowUtils.TransparentContentPane {
            ...
            protected void paintDirect(BufferedImage buf, Rectangle bounds) {
                ...
                HDC screenDC = user.GetDC((HWND)null);
                ...
                try {
                    ...
                    BLENDFUNCTION blend = new BLENDFUNCTION();
                    HWND hWnd = W32WindowUtils.this.getHWnd(win);
                    ...
                    user.ReleaseDC((HWND)null, screenDC);
